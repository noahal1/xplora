...
import { Plus, Loader2, ListTodo, Sparkles, Wand2 } from "lucide-react";
...
export function AddToPlaylistModal({ open, onClose, item, onAdded }: AddToPlaylistModalProps) {
  ...
  const handleCreateAndAdd = useCallback(async () => {
    ...
    setNewCreating(true);
    try {
      const created = await api.createPlaylist({ name: newName.trim() });
      setNewName("");
  ...
  }, [item, newName, newCreating, onAdded, onClose, showToast, t]);
