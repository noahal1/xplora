# 🎬 Xplore — AI Movie Recommender

> AI 驱动的电影推荐系统 · Import your ratings, get smart recommendations
...
Xplore is a full-stack movie recommendation application that uses AI (DeepSeek / OpenAI) to analyze your movie ratings and suggest films you'll love. Import ratings from JSON/CSV files, manage your movie library, and get streaming AI recommendations — all with a sleek dark/light theme UI.

---

## 中文

Xplore 是一个全栈电影推荐应用，利用 AI（DeepSeek / OpenAI）分析你的观影评分，智能推荐你可能喜欢的影片。支持 JSON/CSV 文件导入、电影库管理、流式 AI 推荐，提供暗色/亮色主题切换。
...
> **Database:** Xplore requires PostgreSQL. Set `DATABASE_URL` in `.env`:
> ```
...
|----------|----------|---------|-------------|
| `DATABASE_URL` | **Yes** | — | PostgreSQL connection string, e.g. `postgresql://user:pass@host:5432/xplore` |
...
| `JWT_SECRET` | No | `xplore-dev-secret-...` | JWT signing secret — **change in production** |
...
3. **`ThemeContext`** manages the toggle, persists the choice to `localStorage` (`xplore-theme`), and detects system preference via `prefers-color-scheme`
...
The Docker setup:
- **PostgreSQL 16** as the database (`xplore-db` container) with persistent volume
...
nginx -c /absolute/path/to/xplore/nginx.conf
```
